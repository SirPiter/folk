...
/**
 * HTML Article View class for the Cats component
 *
 * @package		Joomla.Site
 * @subpackage	com_cats
 * @since		1.5
 */
class CatsViewForm extends JView
{
	protected $form;
	protected $item;
	...
	public function display($tpl = null)
	{
		...
		if (empty($this->item->id)) {
			$authorised = $user->authorise('core.create', 'com_cats') || (count($user->getAuthorisedCategories('com_cats', 'core.create')));
		} else {
		...
		if ($this->parameters->get('enable_category') == 1) {
			...
			$category = JCategories::getInstance('Cats')->get($this->parameters->get('catid', 1));
