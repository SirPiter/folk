...
/**
 * This models supports retrieving lists of cats.
 *
 * @package		Joomla.Site
 * @subpackage	com_cats
 * @since		1.6
 */
class CatsModelCats extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	/**
	 * Get the master query for retrieving a list of cats subject to the model state.
	 *
...
//echo nl2br(str_replace('#__','foo_',$query));
	...
	public function getItems()
	{
		...
		$globalParams = JComponentHelper::getParams('com_cats', true);

		// Convert the parameter fields into objects.
		foreach ($items as $item)
		{
			$catParams = new JRegistry;
			$catParams->loadString($item->parameters);

			$item->layout = $catParams->get('layout');

			$item->parameters = clone $this->getState('params');
			$item->parameters->merge($catParams);

			// Compute the asset access permissions.
			// Technically guest could edit an cat, but lets not check that to improve performance a little.
			if ($guest) {
            } else {
				$asset	= 'com_cats.cat.'.$item->id;
			...
			if ($access) {
				// If the access filter has been set, we already have only the cats this user can view.
