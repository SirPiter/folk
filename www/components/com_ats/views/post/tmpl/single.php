...
$item        = $this->item;
$ticket      = F0FModel::getTmpInstance('Tickets', 'AtsModel')->getItem($item->ats_ticket_id);
$attachment  = F0FModel::getTmpInstance('Attachments', 'AtsModel')->getItem($item->ats_attachment_id);
$attachmentErrors = $this->input->get('attachmentErrors', array(), 'array');

$item->original_filename = $attachment->original_filename;
$item->mangled_filename  = $attachment->mangled_filename;
$item->mime_type         = $attachment->mime_type;
$item->att_enabled       = $attachment->enabled;

$category_id = $ticket->catid;
$returnURL   = $this->input->getCmd('returnurl', '');
...
<div class="ats-post <?php echo implode(' ',$customClasses) ?> ats-post-status-<?php echo $item->enabled ? 'published' : 'unpublished' ?>" id="p<?php echo $item->ats_post_id ?>">
	...
	<div class="ats-post-body">
		<div class="ats-post-content">
            ...
            <?php
            if($item->attachments): $shownAttachmentsHeader = false; ?>
                    <?php
	...
	<div class="ats-post-footer">
		...
		<div class="ats-post-edits">
			<?php echo JText::sprintf('COM_ATS_TICKETS_MSG_EDITEDBYON', JUser::getInstance($item->modified_by)->username, AtsHelperFormat::date($item->modified_on) ) ?>
		</div>
		<?php endif; ?>
