...
{
    $jLastOn = $this->ticket->modified_on;
    $lastOn  = AtsHelperFormat::date2($this->ticket->modified_on, '', true);
	$lastBy  = JUser::getInstance($this->ticket->modified_by)->username;
}
else
{
    $jLastOn = $this->ticket->created_on;
	$lastOn  = $createdOn;
    $lastBy  = $createdBy;
}

$jLastOn = new JDate($jLastOn);
$ago = AtsHelperTime::timeAgo($jLastOn->toUnix());
...
?>
<tr id="ats-ticket-<?php echo $this->ticket->ats_ticket_id?>" class="ats-latestopen-row" data-latestopentid="<?php echo $this->ticket->ats_ticket_id ?>" data-latestopensequence="<?php echo $counter ?>">
	<td>
		<h4>
