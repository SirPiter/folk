...
class DocumentsModelDocuments extends JModel
{
	...
	function __construct(){
		...
		$limitstart = $mainframe->getUserStateFromRequest($option.'.documents_limitstart', 'limitstart', 0, 'int');
        ...
        $filter_order_Dir = $mainframe->getUserStateFromRequest( $option.'documents_filter_order_Dir', 'filter_order_Dir', 'asc', 'word' );
        $this->setState('documents_filter_order', $filter_order);
        $this->setState('documents_filter_order_Dir', $filter_order_Dir);
	...
	function getData()
	{
		if (empty( $this->_data )){
			...
			$this->_data = $this->_getList($query, $this->getState('documents_limitstart'), $this->getState('limit'));
		}
		return $this->_data;
	}
 ...
 function getPagination()
  {
 	...
 	if (empty($this->_pagination)) {
 	    ...
 	    $this->_pagination = new JPagination($this->getTotal(), $this->getState('documents_limitstart'), $this->getState('limit') );
 	}
	...
	function getKeywords(){
		if (empty($this->_keywords)) {
			$this->_keywords = $this->getState('documents_keywords')	;
		}
...
//	 	$keywords = utf8_decode(trim($this->get_keywords($keywords)));
		...
		for($i=0;$i<sizeof($keyword);$i++){
			$cadena.="((title LIKE '%".$keyword[$i]."%') OR (doc_comment LIKE '%".$keyword[$i]."%') OR (short_description LIKE '%".$keyword[$i]."%') OR (description LIKE '%".$keyword[$i]."%')) AND ";
		}
		if($i>0){
	...
	function _buildContentOrderBy()
	{
		...
		$option = JRequest::getCmd('option');
        $orderby = '';
        $filter_order     = $this->getState('documents_filter_order');
        $filter_order_Dir = $this->getState('documents_filter_order_Dir');
