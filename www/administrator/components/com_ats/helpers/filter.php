...
class AtsHelperFilter
{
	...
	public static function toSlug($value)
	{
		...
		if (class_exists('JApplication') && method_exists('JApplication', 'stringURLSafe'))
			// Preferentially use JApplication::stringURLSafe
		{
	...
	public static function filterTextHtmlpurifier($text, JUser $user = null)
	{
		...
		$params = self::_getComponentParams();
		if (!class_exists('HTMLPurifier'))
		{
			$hpinclude = $params->get('htmlpurifier_include', 1);
			if ($hpinclude)
			{
		...
		$string = $params->get('htmlpurifier_configstring', '');
		$string = trim($string);
		if (empty($string))
		{
