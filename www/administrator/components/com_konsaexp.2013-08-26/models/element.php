...
class ElementModelElement extends JModel
{
 ...
 function getPagination()
  {
	...
	function getArtistId(){
	 ...
	 function get_where_clause_keywords($keywords){ //germi
		...
		for($i=0;$i<sizeof($keyword);$i++){
			$cadena.="(#__konsa_exp_albums.keywords LIKE '% ".$keyword[$i]." %') AND ";
		}
		if($i>0){
	...
	function _buildQuery()
	{
		...
		if ($artist_id > 0) {
			$where_clause[] = ' #__konsa_exp_albums.artist_id = '.(int) $artist_id;
		}
		
		// Build the where clause of the content record query
		$where_clause = (count($where_clause) ? ' WHERE '.implode(' AND ', $where_clause) : '');

		$query = ' SELECT #__konsa_exp_albums.*,#__konsa_exp_artists.artist_name,#__konsa_exp_format.format_name,#__konsa_exp_type.type_name '
			. ' FROM #__konsa_exp_albums '
			.' LEFT JOIN #__konsa_exp_artists ON #__konsa_exp_artists.id = #__konsa_exp_albums.artist_id ' 
			.' LEFT JOIN #__konsa_exp_format ON #__konsa_exp_format.id = #__konsa_exp_albums.format_id ' 
			.' LEFT JOIN #__konsa_exp_type ON #__konsa_exp_type.id = #__konsa_exp_albums.type_id ' 
			.$where_clause
