<?php

/**
 * @version		2.5.14
 * @package		konsaexp
 * @copyright	2014 sirpiter.ru
 * @license		GPLv2
 */

// No direct access
defined( '_JEXEC' ) or die( 'Restricted access' );

class ArtistsControllerArtist extends ArtistsController
{

	function __construct()
	{
		parent::__construct();

		// Register Extra tasks
		$this->registerTask( 'add'  , 	'edit' );
		$this->registerTask( 'apply',	'save' );
		$this->registerTask( 'remove_session',	'remove_session' );
	}

	function edit()
	{
		JRequest::setVar( 'view', 'artist' );
		JRequest::setVar( 'layout', 'form'  );
		JRequest::setVar('hidemainmenu', 1);

		parent::display();
	}

	function save()
	{
		$model = $this->getModel('artist');
		if ($model->store($post)) {
			$msg = JText::_( 'COM_KONSAEXP_ARTIST_SAVED' );
		} else {
			$msg = JText::_( 'COM_KONSAEXP_ERROR_SAVING_ARTIST' );
		}

		$task = JRequest::getCmd( 'task' );
		$id = JRequest::getVar('id');

		switch ($task)
		{
			case 'apply':
				$link = 'index.php?option=com_konsaexp&controller=artist&task=edit&cid[]='. $id ;
				break;

			case 'save':
			default:
				$link = 'index.php?option=com_konsaexp&controller=artists';
				break;
		}

		$this->setRedirect($link, $msg);
	}

	function remove()
	{
		$model = $this->getModel('artist');
		if(!$model->delete()) {
			$msg = JText::_( 'COM_KONSAEXP_ARTISTS_NOT_DELETED' );
		} else {
			$msg = JText::_( 'COM_KONSAEXP_ARTISTS_DELETED' );
		}

		$this->setRedirect( 'index.php?option=com_konsaexp&controller=artists', $msg );
	}

	function cancel()
	{
		$msg = JText::_( 'COM_KONSAEXP_OPERATION_CANCELED' );
		$this->setRedirect( 'index.php?option=com_konsaexp&controller=artists', $msg );
	}
	
	function remove_session()
	{
		$artist_id = JRequest::getVar('id');
		$model = $this->getModel('artist');
		if(!$model->delete_session()) {
			$msg = JText::_( 'COM_KONSAEXP_ARTISTS_TO_SESSION_NOT_DELETED' );
		} else {
			$msg = JText::_( 'COM_KONSAEXP_ARTISTS_TO_SESSION_DELETED' );
		}
		If ($artist_id) {
			$this->setRedirect( 'index.php?option=com_konsaexp&controller=artist&task=edit&cid[]=' . $artist_id, $msg );
		} else {
			$this->setRedirect( 'index.php?option=com_konsaexp&controller=artists', $msg );
		}
	}
		
}